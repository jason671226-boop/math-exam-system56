...
def derive_answer(q: ConfirmedQuestion, provider: Callable[[list[Any]],str], *, provider_name='gemini')->DerivedAnswerResult:
    import os
    q.validate()
    if os.getenv("MATHAI_DERIVED_ANSWER_MOCK", "").strip().lower() in {"1", "true", "yes", "on"}:
        return DerivedAnswerResult("SUCCESS", "6", "6", "C", 1.0, "移項後得 2x=12，因此 x=6。", "代回 2×6+5=17，成立。", False, (), "LOCAL_MOCK", False, None)
