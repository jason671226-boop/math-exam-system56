...
# 嘗試載入 Google GenAI 套件 (支援新版 google-genai 與舊版 google-generativeai)
try:
        ...
        if GENAI_MODE == "new":
            ...
            for pref in ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash", "gemini-1.5-pro"]:
                if pref in available:
                    st.session_state["working_gemini_model"] = pref
                    return pref
            if available:
                st.session_state["working_gemini_model"] = available[0]
                return available[0]
        else:
...
def call_gemini_api(contents):
    if not GENAI_AVAILABLE:
        raise Exception("尚未安裝 Google GenAI 套件。")
    
    # 取得當前 API Key 權限下真實可用的模型名稱
    primary_model = get_working_gemini_model()
    candidate_models = [primary_model, "gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash", "gemini-1.5-pro"]
    ...
    for m_name in unique_candidates:
        try:
            if GENAI_MODE == "new":
                client = genai.Client(api_key=GEMINI_KEY)
                res = client.models.generate_content(model=m_name, contents=contents)
                if res and res.text:
                    st.session_state["working_gemini_model"] = m_name
                    return res.text
            else:
                genai.configure(api_key=GEMINI_KEY)
            ...
            if "404" in err_str or "not found" in err_str or "not supported" in err_str:
                if "working_gemini_model" in st.session_state and st.session_state["working_gemini_model"] == m_name:
                    del st.session_state["working_gemini_model"]
                continue
            else:
                raise e
    ...
    if last_err:
        raise last_err
    raise Exception("無法從 AI 伺服器取得回應。")
...
# ==========================================
    ...
    with tab_scan:
        ...
        def perform_ai_scan(files, mode="normal"):
            ...
            if not GENAI_AVAILABLE:
                st.error("⚠️ 系統偵測不到 Google GenAI 相關套件，請確認伺服器環境！")
                return False
            ...
            if GENAI_AVAILABLE and PIL_AVAILABLE and GEMINI_KEY:
                try:
                    if mode == "loose":
                        prompt = (
                            "你是一個資深的數學老師與考卷辨識專家。\n"
                            "請對圖片中的數學考卷進行精準的圖文辨識，提取出題目內容（包含完整的數學公式與符號）。\n"
                            "【錯題挑選嚴格定義】：請只擷取符合以下條件的錯題，其餘題目請忽視：\n"
                            "1. 題目附近有紅筆加註或批改痕跡\n"
                            "2. 留空白未填寫作答的地方\n"
                            "3. 被打叉（X）或劃錯記號的地方\n"
                            "⚠️ 極度重要：如果有「打勾（✓）」的地方代表答對，請絕對不要列入！\n"
                        )
                    else:
                        prompt = (
                            "你是一個資深的數學老師與考卷辨識專家。\n"
                    ...
                    if is_trial:
                        prompt += "【數量限制】：這是試用請求，請精準控管，最多只需要萃取 5 道符合條件的題目即可。\n"
                    ...
                    res_text = call_gemini_api(contents)
                    if res_text:
                        st.session_state["scanned_text"] = res_text.strip()
                        if is_trial:
                ...
                st.error("⚠️ 系統缺少必要的影像處理套件或 API 金鑰！")
                return False
            return False
