...
def fixture(status="staged"):
    profile = "CURRICULUM_V27:PREHIGH:G6:COMMON"
    release = "CURRICULUM_V27_EA0E6735"
    return Client(
        {
            "curriculum_releases": [
                {"release_id": release, "status": status, "is_active": False}
            ],
            "curriculum_profiles": [
                {
                    "release_id": release,
...
class SupabaseRuntimeTests(unittest.TestCase):
    ...
    def test_live_accepts_verified(self):
        runtime = select_curriculum_runtime_v27(
            object(), fixture("verified"), source="supabase"
        )
        self.assertEqual(runtime.validate()["release_status"], "verified")
