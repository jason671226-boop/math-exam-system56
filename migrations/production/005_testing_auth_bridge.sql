...
--
-- Architecture: Supabase Native Email OTP + Postgres Send Email Hook.
-- NO service_role client usage, NO temporary password, NO password rotation,
-- NO synthetic JWT, NO deterministic UUID.
--
-- Flow
-- ----
-- 1. Browser calls mathai_testing_auth_prepare(email, sha256(nonce), expires_at)
--    to create one open pre-claim row for an allowlisted test account.
-- 2. Browser calls the official supabase.auth.sign_in_with_otp endpoint
--    (should_create_user = false).
-- 3. Supabase generates the REAL 6-digit OTP and invokes the Postgres
--    Send Email Hook below instead of SMTP for allowlisted accounts.  The
--    hook stores email_data.token / token_hash into the open pre-claim row
--    and returns a valid empty response, so no Email is sent.
-- 4. Browser calls mathai_testing_auth_reveal(email, sha256(nonce)) to read
--    that single OTP and displays it on screen.
-- 5. Browser calls the official supabase.auth.verify_otp endpoint.  On success
--    GoTrue establishes the REAL Auth session; on failure the app calls
--    mathai_testing_auth_fail to increment attempt_count.
-- 6. After a successful session the app calls mathai_testing_auth_consume to
--    consume/clear the challenge.
--
...
-- Client roles never read/write the table directly; only the SECURITY DEFINER
-- RPCs below and the Send Email Hook (supabase_auth_admin) touch it.
revoke all on table public.testing_auth_challenges from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- Narrow SECURITY DEFINER RPCs.  Every function fixes search_path and only
-- accepts an allowlisted Email plus the matching sha256 challenge hash.  No
-- function lists rows or returns another account's challenge.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Postgres Send Email Hook (executed by supabase_auth_admin).
-- Configure in Supabase Dashboard -> Authentication -> Hooks -> Send Email
-- Hook, pointing at public.mathai_testing_auth_email_hook.  The exact payload
-- and return shape follow the Supabase Auth Hooks contract of the deployed
-- project version; for allowlisted accounts the hook returns a valid empty
-- jsonb response to take over Email delivery without sending SMTP Email.
-- ---------------------------------------------------------------------------
...
begin
  if v_email not in (
    ...
    -- Non-allowlisted accounts must not receive a displayed OTP and the app
    -- does not offer the testing path for them.  Raising blocks the /otp
    -- request so no Email is sent through this hook.
