-- PROPOSAL ONLY. DO NOT EXECUTE WITHOUT STAGING APPROVAL.
-- Ownership architecture: auth.users -> student_access -> learning_students.
...
revoke all on public.diagnostic_item_results from anon;
revoke all on public.knowledge_mastery from anon;
revoke all on public.thinking_skill_evidence from anon;
...
-- Intentionally no INSERT/UPDATE/DELETE policy on student_access for clients.
-- Intentionally no DELETE policy on student learning tables in Private Beta.
